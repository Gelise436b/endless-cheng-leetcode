...
- [📅Day 16 - 组合数学与状态压缩：破解“复杂条件”滑窗](#day-16---组合数学与状态压缩破解复杂条件滑窗)
- [📅Day 17 - 奇偶性与定值滑窗：数学思维与“恰好K”模型](#day-17---奇偶性与定值滑窗数学思维与恰好K模型)
- [📅Day 18 - 恰好K的终极形态与“单调性方向”陷阱](#day-18---恰好K的终极形态与单调性方向陷阱)
---
...
---
### Day 17 - 奇偶性与定值滑窗：数学思维与“恰好K”模型

* **完成题目**：
* [x] **[2495] 统计乘积为偶数的子数组数目**（思维转换：从“滑窗”转向“数学贡献法”，利用奇偶性快速计数）
* [x] **[930] 和相同的二元子数组**（经典模型：彻底搞懂了为什么“恰好 K”要转化成“至多 K”减去“至多 K-1”）


* **学习重点**：
* **数学思维与贡献法 ([2495])**
* **误区**：试图用滑窗去维护乘积，结果发现数字太大或者窗口收缩逻辑复杂。
* **正解**：乘积为偶数  **子数组里至少有一个偶数**。
* **逆向思维**：或者算 `总子数组数 - 全是奇数的子数组数`。
* **线性扫描法**：记录上一个偶数出现的位置 `last_even`。当前位置 `i` 结尾的合法子数组个数 = `last_even + 1`（下标从0开始）。


* **定值滑窗模型 ([930])**
* **黑话**：“求和恰好为 `goal` 的子数组”。
* **痛点**：因为有 0 的存在，窗口和不单调，普通的 `while (sum > goal)` 没法处理 `sum == goal` 时还需要统计多少个 0 的情况。
* **万能公式**：**`Three Pointer`** 或者 **`atMost(goal) - atMost(goal - 1)`**。
* **转换**：把“等于”问题转化为两个“小于等于”问题的差。这样做的好处是 `atMost` 函数里的窗口逻辑是严格单调的（只缩不扩），非常好写。




* **代码逻辑深度解析 ([930] 的 atMost 通用解法)**：
* 这套逻辑是解决 1248, 992 等题目的基石。
* **定义函数**：`helper(nums, goal)` 计算和 **<=** goal 的子数组数量。
* **进窗口 (`right`)**：
* `sum += nums[right]`：常规累加。


* **出窗口 (`left`)**：
* `while (left <= right && sum > goal)`：只要爆了就缩。注意这里不需要管 `sum == goal` 的情况，统统算在合法里。
* `sum -= nums[left++]`。


* **统计**：
* `ans += right - left + 1`：这一步是核心。以 `right` 结尾，`left` 开头的所有子数组都满足条件。


* **最终答案**：`helper(nums, goal) - helper(nums, goal - 1)`。


* **卡点反思**：
* **对 0 的处理不够敏感 ([930])**：
* **现象**：一开始想手写 `sum == goal` 的逻辑，结果遇到 `[0,0,0]` 这种全是 0 的情况，代码写了一堆 `if-else` 处理左边的 0。
* **对策**：遇到“包含 0”或者“负数”导致单调性失效的情况，要么用 **前缀和+哈希表**，要么用 **atMost 差分**。


* **思维定势于滑窗 ([2495])**：
* **现象**：看到“连续子数组”就想套 `left, right` 双指针。
* **对策**：如果题目只关心情境（比如奇偶、整除），且不需要具体窗口内容，优先考虑 **DP** 或 **数学规律（贡献法）**。
...
* **明日计划（Day 18）**：
* **复习巩固**：
* 手写一遍 **[930]** 的 `atMost` 模板，确保肌肉记忆。
* 复习 **[2495]** 的 `last_even` 逻辑，思考如果变成“乘积为奇数”怎么改。


* **新题攻坚（定值滑窗进阶 & 困难题）**：
* **[1248] 统计「优美子数组」**（[930] 的孪生兄弟，把奇数当 1 偶数当 0 就是同一道题）
* **[3306] 元音辅音计数 II**（可能是滑窗+哈希表的综合大题，考察多条件限制）
* **[992] K 个不同整数的子数组**（Hard 题，`atMost` 思想的终极应用，必须要掌握）
---

### Day 18 - 恰好K的终极形态与“单调性方向”陷阱

* **完成题目**：
* [x] **[1248] 统计「优美子数组」**（秒杀：完全等同于 [930]，奇数看作1，偶数看作0，标准 `atMost` 模板解决）
* [x] **[992] K 个不同整数的子数组**（Hard 拿下：虽然是 Hard，但完全符合 `atMost(K) - atMost(K-1)` 的逻辑，顺利解决）
* [x] **[3306] 元音辅音字符串计数 II**（**重难点**：遭遇了滑窗逻辑的滑铁卢，通过深度复盘理解了“约束方向”的重要性）


* **学习重点**：
* **模板的普适性验证 ([1248], [992])**
* 验证了 `恰好 K = atMost(K) - atMost(K-1)` 在常规计数问题上的威力。
* 无论是“奇数个数”还是“不同整数个数”，本质都是 **“越短越容易满足（或保持不变）”** 的上限约束，所以标准的收缩窗口逻辑（`right - left + 1`）完全适用。


* **单调性的方向冲突 ([3306])**
* **痛点**：这道题不能简单套用 `atMost`。
* **冲突源头**：题目有两个约束。
1. **辅音数量**：如果是“至多 K”，则越短越好（AtMost）。
2. **元音种类**：**必须包含 5 种**，这是下限约束，**越长越好**（AtLeast）。


* **后果**：传统的 `while` 收缩窗口会破坏“元音完整性”，导致 `right - left + 1` 统计公式失效（会把缺元音的短子数组算进去）。


* **双解法领悟 ([3306])**
* **正解逻辑（AtLeast）**：将问题转化为 `atLeast(K) - atLeast(K+1)`。
* 利用 **“辅音至少 K”** 和 **“元音必须 5 个”** 都是 **“越长越好”** 的特性，统一了单调性。
* 统计公式变为 `ans += left`（统计所有合法的外部起点）。


* **补集逻辑（歪打正着）**：虽然写的是 `atMost` 且用了 `right - left + 1`，但实际上是在统计 **“不合法的子数组”**（失败者）。
* 利用 `Fail(K+1) - Fail(K)` 反向得出了中间的合法数量。这是一次对容斥原理的深刻实践。
...
* **代码逻辑深度解析 ([3306] 的 AtLeast 视角)**：
* 这是解决混合约束（既有上限又有下限）的关键。
* **进窗口**：统计元音和辅音。
* **判断**：`while (cnt1.size() == 5 && cnt2 >= k)` —— 注意这里是 `>=`，找的是合法窗口。
* **核心动作**：
* `left++`：不断尝试收缩，直到**破坏**合法性为止。
* `ans += left`：核心差异！因为 `[0...left-1]` 都是合法的起点，它们对应的子数组更长，不仅辅音够，元音也绝对齐。
...
* **卡点反思**：
* **思维定势 ([3306])**：
* **现象**：看到“恰好 K”就无脑套 `atMost(K) - atMost(K-1)`，忽略了题目中“必须包含 5 种元音”这个硬性条件对窗口收缩的限制。
* **本质**：没分清 **上限约束**（越短越好）和 **下限约束**（越长越好）的区别。
* **教训**：
* 如果题目全是上限约束（如 1248, 992, 930）-> 用 `atMost` (公式 `right - left + 1`)。
* 如果题目有下限约束（如 3306）-> 必须用 `atLeast` (公式 `ans += left`)，或者用补集思维。
...
* **明日计划（Day 19）**：
* **深度复习**：
* 手写 **[3306]** 的 `atLeast` 版本，彻底纠正思维惯性。确保理解为什么是 `ans += left`。


* **新题攻坚（特殊滑窗技巧）**：
* **[825] 适龄的朋友**（可能会用到排序+双指针/二分，考察对条件的转化）。
* **[2401] 最长优雅子数组**（位运算 + 滑窗，考察按位与的性质）。
* **[1156] 单字符重复子串的最大长度**（滑窗 + 分组循环，考察对“允许交换一次”的处理）。
